...
class Blip2Qformer(Blip2Base):
    ...
    def forward(self, samples):
        ...
        print(f'image_embeds size after vision projection: {image_embeds.size()}, dtype: {image_embeds.dtype}')
        ...
        # Ensure the token indices do not exceed the vocabulary size
        vocab_size = self.tokenizer.vocab_size
        text_tokens = torch.clamp(text_tokens, max=vocab_size - 1)
        ...
        lang_feat_mask = self.MCAN.make_mask(text_tokens.unsqueeze(2)).to(torch.bool)
        img_feat_mask = self.MCAN.make_mask(image_embeds).to(torch.bool)
        print(f'lang_feat_mask size: {lang_feat_mask.size()}')
        print(f'img_feat_mask size: {img_feat_mask.size()}')

        print('=============================================')

        # Ensure the mask values are valid
        print(f'lang_feat_mask unique values: {torch.unique(lang_feat_mask)}')
        print(f'img_feat_mask unique values: {torch.unique(img_feat_mask)}')
